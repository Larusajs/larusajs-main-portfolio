...
import { Mail, Plus, ChevronRight, Copy, Check, ExternalLink, Layers, Briefcase, Terminal, Code, Database, Globe } from 'lucide-react';
import { Card } from './components/Card';
import { SectionHeader } from './components/SectionHeader';
import { PERSONAL_INFO, PROJECTS, EXPERIENCES, SOCIAL_LINKS, PUBLICATIONS } from './constants';
...
const App: React.FC = () => {
  ...
  return (
    <div className="min-h-screen pb-20 pt-10 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto flex flex-col gap-8">
      ...
      <main className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-8 flex flex-col gap-10">
          ...
          {/* Hero Section */}
          <section>
            <Card className="p-8 md:p-14 relative overflow-hidden">
              <div className="flex justify-between items-start mb-14">
                <div className="flex items-center gap-2 px-3 py-1 bg-[#1a1a1a] rounded-full border border-[#262626] inner-shadow">
                ...
                {PERSONAL_INFO.availability && (
                  <div className="flex items-center gap-2 bg-emerald-500/5 border border-emerald-500/20 rounded-full px-3 py-1">
                    ...
                    <span className="text-emerald-500 text-[10px] font-bold uppercase tracking-widest">Available</span>
                  </div>
                )}
              </div>
              ...
              <div className="flex flex-col md:flex-row gap-12 items-center justify-between text-center md:text-left">
                <div className="flex-1 order-2 md:order-1 z-10">
                  <h1 className="text-4xl sm:text-5xl lg:text-7xl font-bold mb-6 tracking-tight leading-[1.05]">I'm {PERSONAL_INFO.name}</h1>
                  <p className="text-gray-400 text-lg leading-relaxed mb-10 max-w-xl mx-auto md:mx-0">
                    {PERSONAL_INFO.summary} Currently in {PERSONAL_INFO.location}.
                  </p>
                  <div className="flex flex-row gap-4 justify-center md:justify-start">
                    <button className="flex items-center gap-2 bg-white text-black px-8 py-4 rounded-2xl text-sm font-bold transition-all hover:bg-gray-200 active:scale-95 shadow-lg whitespace-nowrap">
                      <Plus size={16} strokeWidth={2.5} />
                      Let's Talk
                    </button>
                    <button 
                      onClick={handleCopyEmail}
                      className="flex items-center gap-2 bg-[#1a1a1a] hover:bg-[#222] text-white px-8 py-4 rounded-2xl text-sm font-bold transition-all border border-[#262626] inner-shadow active:scale-95 whitespace-nowrap"
                    >
                ...
                <div className="relative group order-1 md:order-2 shrink-0">
                  {/* Decorative background glow that stays contained within Card overflow-hidden */}
                  <div className="absolute -inset-10 bg-white/5 rounded-full blur-[80px] opacity-50 transition-opacity duration-1000"></div>
                  
                  <div className="relative w-48 h-48 sm:w-56 sm:h-56 md:w-60 md:h-60 lg:w-72 lg:h-72 rounded-full p-2 bg-gradient-to-br from-[#262626] via-[#1a1a1a] to-[#0f0f0f] border border-[#262626] shadow-2xl flex items-center justify-center">
                    <div className="w-full h-full rounded-full overflow-hidden border border-[#333]">
                      <img 
                        src='./enes-foto.jpeg'
                        alt="Profile" 
                        className="w-full h-full rounded-full object-cover grayscale brightness-90 hover:brightness-100 hover:scale-105 transition-all duration-700 ease-out"
                      />
                    </div>
                  </div>
                </div>
        ...
        <div className="lg:col-span-4 flex flex-col gap-10">
          ...
          <section>
            ...
            <Card className="p-8 flex items-center justify-between">
              <div className="flex gap-3">
                {SOCIAL_LINKS.map((link) => (
                  <a 
                    key={link.platform} 
                    href={link.url}
                    className="w-12 h-12 flex items-center justify-center bg-[#1a1a1a] text-gray-500 rounded-2xl hover:text-white hover:bg-[#222] transition-all border border-[#262626] inner-shadow active:scale-90"
              ...
              <a href={`mailto:${PERSONAL_INFO.email}`} className="text-gray-600 hover:text-white transition-colors p-2">
                <Mail size={24} />
              </a>
            </Card>
          </section>
